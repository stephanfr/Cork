...
namespace Cork
{
    namespace Intersection
    {
        class PerturbationEpsilon
        {
           public:
            ...
            AdjustPerturbationResult adjust()
            {
                ...
                if (!sufficientRange())
                {
                    return (AdjustPerturbationResult::failure(
                        AdjustPerturbationResultCodes::MAXIMUM_PERTURBATION_REACHED, "Maximum Perturbation reached"));
                }

                return (AdjustPerturbationResult(m_numAdjustments));
            }

            Math::Vector3D getPerturbation() const
            {
        ...
        inline Math::Vector3D computeCoords(const TopoEdge& e, const TopoTri& t,
        ...
        class GenericVertType : public boost::noncopyable, public IntrusiveListHook
        {
           public:
            ...
            GenericVertType(VertexType type, const Math::Vector3D& coordinate, bool boundary,
                            GluePointMarker& glueMarker)
        ...
        class GenericEdgeType : public boost::noncopyable, public IntrusiveListHook
        {
           public:
            ...
            GenericEdgeType(EdgeType type, bool boundary, GenericVertType* endpoint)
        ...
        class IntersectionProblemBase : public TopoCache
        {
           public:
            ...
            {
               public:
                MessageType type() const { return (MessageType::END_OF_MESSAGES); }
            };
            ...
            class TriangleAndIntersectingEdgesMessage : public TriAndEdgeQueueMessage
            {
               public:
                ...
                MessageType type() const { return (MessageType::TRI_AND_INTERSECTING_EDGES); }
            ...
            {
                return (
                    m_isctVertTypeList.emplace_back(GenericVertType::VertexType::INTERSECTION, coords, false, glue));
            }
            ...
            {
                return (
                    m_isctEdgeTypeList.emplace_back(GenericEdgeType::EdgeType::INTERSECTION, false, endpoint, tri_key));
            }

            OrigVertType* newOrigVert(TopoVert* v)
            {
                return (m_origVertTypeList.emplace_back(GenericVertType::VertexType::ORIGINAL, *v,
                                                        *(v->quantizedValue()), true));
            }
        ...
        class TriangleProblem : public boost::noncopyable, public IntrusiveListHook
        {
           public:
            ...
            IsctVertType* addBoundaryPointAlone(const TopoEdge& edge, const Math::Vector3D& coord,
                                                GluePointMarker& glue)
            {
                IsctVertType* iv = m_iprob.newSplitIsctVert(coord, glue);
                addBoundaryHelper(edge, iv);
                ...
                for (IsctEdgeType* ie : m_iedges)
                {
                    if (ie->ends()[1] == nullptr)
                    {
                        ...
                        if (!m_triangle.findCommonVertex(ie->otherTriKey().value(), vert))
                        {
                            ...
                            std::cout << "degen count:" << m_iprob.ExactArithmeticContext().degeneracy_count
                                      << std::endl;
                            std::cout << "exact count: " << m_iprob.ExactArithmeticContext().exact_count << std::endl;
#endif

                            return (ConsolidateResult::failure(
                                ConsolidateResultCodes::COULD_NOT_FIND_COMMON_VERTEX,
        ...
        bool IntersectionProblem::findTriTriTriIntersections()
        {
            ...
            for (auto& tprob : m_triangleProblemList)
            {
                ...
                for (IntersectionEdgePointerList::iterator ie1 = tprob.iedges().begin(); ie1 != tprob.iedges().end();
                     ie1++)
                {
                ...
                if (!foundIntersections)
                {
                    if (result.error_code() == TryToFindIntersectionsResultCodes::OUT_OF_MEMORY)
                    {
                        return (IntersectionProblemResult::failure(
                            result, IntersectionProblemResultCodes::OUT_OF_MEMORY, "Out of Memory"));
                    }
        ...
        IntersectionProblem::IntersectionProblemResult IntersectionProblem::ResolveAllIntersections()
        {
            ...
            for (auto& tprob : m_triangleProblemList)
            {
                ...
                if (!result.succeeded())
                {
                    ...
                    {
                        return (IntersectionProblemResult::failure(
                            result, IntersectionProblemResultCodes::SELF_INTERSECTING_MESH,
...
//
//	Create the IntersectionProblemWorkspace Factory down here as it has to follow declaration of the workspace and be in
// global scope
//
...
//{
//	template <>
//	SEFUtility::CachingFactory<Intersection::IntersectionProblemWorkspace>::CacheType<Intersection::IntersectionProblemWorkspace>
// SEFUtility::CachingFactory<Intersection::IntersectionProblemWorkspace>::m_cache;
//}
