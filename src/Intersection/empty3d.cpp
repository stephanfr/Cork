...
namespace Cork
{
    namespace Empty3d
    {
        ...
        inline void toExt(Ext4_1& out, const Cork::Math::Vector3D& in)
        {
            out.e0 = in.x();
            out.e1 = in.y();
            out.e2 = in.z();
        ...
        inline void toVec3d(Cork::Math::Vector3D& out, const Ext4_1& in)
        {
        ...
        inline void toFixExt(FixExt4_1<IN_BITS>& out, const Cork::Math::Vector3D& in,
        ...
        int TriEdgeIn::emptyFilter() const
        {
            Ext4_2 temp2;
            AbsExt4_2 ktemp2;
            std::array<Ext4_1,2> ep;
            std::array<AbsExt4_1,2> kep;
            std::array<Ext4_1,3> tp;
        ...
        bool TriTriTriIn::isEmpty(ExactArithmeticContext& context) const
        {
            ...
            std::array<std::array<Ext4_1,3>,3> tps;
            ...
            for (uint ti = 0; ti < 3; ti++)
            {
                ...
                {  // three copies...
                    Ext4_3 a;
                    Ext4_2 temp_e2;  // help out liveness analysis
                    join(temp_e2, ((pi == 0) ? p_isct : tps[ti][0]), ((pi == 1) ? p_isct : tps[ti][1]));
        ...
        int TriTriTriIn::emptyFilter() const
        {
            Ext4_2 temp2;
            AbsExt4_2 ktemp2;
            std::array<std::array<Ext4_1,3>,3> p;
            ...
            for (uint i = 0; i < 3; i++)
            {
                toExt(p[i][0], m_tri[i].p0);
                abs(kp[i][0], p[i][0]);
                toExt(p[i][1], m_tri[i].p1);
                abs(kp[i][1], p[i][1]);
                toExt(p[i][2], m_tri[i].p2);
                abs(kp[i][2], p[i][2]);
